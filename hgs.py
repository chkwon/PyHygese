...
def test(n):
    ...
    # data preparation
    cvrp_data = {}
    cvrp_data['x_coordinates'] = x
    cvrp_data['y_coordinates'] = y

    # solve TSP using coordinates only
    result1 = hgs_solver.solve_tsp(cvrp_data)
    ...
    cvrp_data['distance_matrix'] = dist_mtx
    result2 = hgs_solver.solve_tsp(cvrp_data)

    # solve TSP via CVRP function using coordinates only
    cvrp_data['service_time'] = np.zeros(n)
    cvrp_data['demand'] = np.ones(n)
    cvrp_data['vehicle_capacity'] = n
    cvrp_data['num_vehicles'] = 1
    cvrp_data['distance_matrix'] = None

    result3 = hgs_solver.solve_cvrp(cvrp_data)

    print(result1)
    print(result2)
    print(result3)
    assert(result1.cost == result2.cost == result3.cost)
